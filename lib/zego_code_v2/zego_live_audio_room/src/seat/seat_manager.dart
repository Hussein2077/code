...
class ZegoLiveSeatManager {
 ...
 // to my server
  ...
  //   }
  //
  //   //to forbidden normal use seat in lock seats
  //   if(RoomScreen.listOfLoskSeats.containsKey(index)
  //       && RoomScreen.listOfLoskSeats[index]!=0&&
  //       !RoomScreen.adminsInRoom.containsKey(localUserID)&&
  //       ZegoLiveAudioRoomRole.host != localRole.value){
  //     return false;
  //   }
  //
  //   if (-1 != getIndexByUserID(localUserID)) {
  ...
  //         userId: localUserID,
  //         position: index.toString()));
  //     OwnerDataModel userData = await getUserData(getUserByIndex(index)!.id);
  //     RoomScreen.usersInMic.putIfAbsent(index.toString(), () => userData);
  //      log( RoomScreen.usersInMic.toString());
  //     if(RoomScreen.listOfMuteSeats.containsKey(index)){
  //       ZegoUIKit().turnMicrophoneOn(false,userID: getUserByIndex(index)?.id.toString());
  //     }
  //
