...
class AcountBloc extends Bloc<AccountEvents, AccountStates> {
  ...
  AcountBloc({required this.boundPlatformUC, required this.deleteAccountUC})
  ...
  FutureOr<void> deleteAccount(
      DeleteAccountEvent event, Emitter<AccountStates> emit) async {
    emit(DeleteAccountLoading());
    final result = await deleteAccountUC.call(const Noparamiter());

    result.fold(
        (l) => emit(DeleteAccountSuccessState(successMessage: l)),
        (r) => emit(DeleteAccountErrorState(
  ...
  FutureOr<void> boundNubmer(
      BindNumberAccountEvent event, Emitter<AccountStates> emit) async {
    emit(NumberAccountLoading());
    final result = await boundPlatformUC.boundNumber(BoundNumberPramiter(
    ...
    result.fold(
        (l) => emit(NumberAccountSuccessState(successMessage: l)),
        (r) => emit(NumberAccountErrorState(
            errorMessage: DioHelper().getTypeOfFailure(r))));
  }

  FutureOr<void> changeNubmer(
      ChangeNumberAccountEvent event, Emitter<AccountStates> emit) async {
    emit(ChangeNumberLoading());
    final result = await boundPlatformUC.changePhone(BoundNumberPramiter(
    ...
    result.fold(
        (l) => emit(ChangeNumberSuccessState(successMessage: l)),
        (r) => emit(ChangeNumberErrorState(
            errorMessage: DioHelper().getTypeOfFailure(r))));
  }

  FutureOr<void> changePassword(
      ChangePasswordAccountEvent event, Emitter<AccountStates> emit) async {
    emit(ChangePasswordLoading());
    final result = await boundPlatformUC.changePassword(BoundNumberPramiter(
      password: event.password,
    ...
    result.fold(
        (l) => emit(ChangePasswordSuccessState(successMessage: l)),
        (r) => emit(ChangePasswordErrorState(
            errorMessage: DioHelper().getTypeOfFailure(r))));
  }

  FutureOr<void> boundGoogle(
      BindGoolgeAccountEvent event, Emitter<AccountStates> emit) async {
    emit(GoogleAccountLoading());
    final result = await boundPlatformUC.boundGmail();
    result.fold(
        (l) => emit(GoogleAccountSuccessState(successMessage: l)),
        (r) => emit(GoogleAccountErrorState(
            errorMessage: DioHelper().getTypeOfFailure(r))));
  }
}
