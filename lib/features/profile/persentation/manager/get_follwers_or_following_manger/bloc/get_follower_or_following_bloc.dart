...
class GetFollowerOrFollowingBloc extends Bloc<BaseGetFollowerOrFollowingEvent, GetFollowerOrFollowingState> {
 final GetFriendsOrFollowersUsecase getFriendsOrFollowersUsecase ; 
  bool isLoadingMore = false ;
  GetFollowerOrFollowingBloc({required this.getFriendsOrFollowersUsecase}) : super(GetFollowerOrFollowingInitial(null)) {
    on<GetFriendsOrFollowersOrFollwothemEvent>((event, emit) async{

       emit( GetFollowerOrFollowingLoadingState(null));
      final result = await getFriendsOrFollowersUsecase.getFriendsOrFollowers(type: event.type);

      result.fold(
          (l) { emit(GetFollowerOrFollowingErrorState(null,
     ...
     on<GetMoreFriendsOrFollowersOrFollwothemEvent>((event, emit) async {
               ...
               isLoadingMore = true ;
              emit(GetFollowerOrFollowingSucssesState(data: [...state.data! ,]));

     
       var result = await getFriendsOrFollowersUsecase.getFriendsOrFollowers(type: event.type , page: event.page);
      result.fold((l) {
        isLoadingMore = false ;
         emit(GetFollowerOrFollowingErrorState(null,
        DioHelper().getTypeOfFailure(l)
      ));
      }, (r) {
     ...
     if(r!=[]){        emit(GetFollowerOrFollowingSucssesState(data: [...state.data! , ...r]));
}
      });
    });
  }
