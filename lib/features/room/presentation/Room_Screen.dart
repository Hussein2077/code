...
class RoomScreenState extends State<RoomScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (zegoList.length>RoomScreen.usersInRoom.length){
      //add users
      getUsersDataInRoomNew(zegoList);

    }
    else if(zegoList.length<RoomScreen.usersInRoom.length){
       Map<String, UserDataModel> tempUsersInRoom = {};
       for(int i = 0 ; i < zegoList.length; i++){
        if(RoomScreen.usersInRoom.containsKey(zegoList[i].id)){
        tempUsersInRoom
        .putIfAbsent(zegoList[i].id.toString(), (){
      return RoomScreen.usersInRoom[zegoList[i].id.toString()]!;
    });
       ...
       RoomScreen.usersInRoom=tempUsersInRoom;
 ...
 yellowBannercontroller = AnimationController(
    ...
    offsetAnimationBanner = Tween(begin:const Offset(-650, 0),
        end:  Offset(ConfigSize.defaultSize!*1.5, 0)).animate(CurvedAnimation(
  ...
  //   RoomScreen.zegoMediaPlayer = ZegoLiveAudioRoomController();
   RoomScreen.myCoins.value = widget.myDataModel.myStore?.coins.toString()??'' ;
   if (!MainScreen.iskeepInRoom.value) {
     RoomScreen.roomIsLoked = widget.room.roomPassStatus!;


     Future.delayed(Duration.zero, () async {
       await getUsersDataInRoomNew(ZegoUIKit().getAllUsers());
     });
     ...
     if (widget.myDataModel.intro! != "") {
       loadAnimationEntro(widget.myDataModel.introId?.toString() ?? "",widget.myDataModel.intro!);
     ...
     for (int i = 0; i < widget.room.seats!.length; i++) {
       if (widget.room.seats![i] == "locked") {
         ...
         ZegoUIKitUser zegoUIKitUser = ZegoUIKitUser(id: myDataModel.id.toString(),
             name: myDataModel.name.toString());
         zegoUIKitUser.inRoomAttributes.value['uu']='moooo';
         RoomScreen.userOnMics.value
             .putIfAbsent(i, () => zegoUIKitUser);
       }
     }
               ...
               builder: (BuildContext context) {
                 return AlertDialog(
                   ...
                   content: DialogLuckyBox(
                     coins: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes.length -
                         1].coinns,
                     luckyBoxId: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].boxId,
                     ownerBoxId: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].ownerBoxId,
                     ownerBoxName: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].ownerName,
                     removeController: luckyBoxRemovecontroller,
                     typeLuckyBox: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].typeLuckyBox,
                     remTime: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].remTime,
                     ownerImage: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes
                         .length - 1].ownerImage,
                     uid: RoomScreen.luckyBoxes[RoomScreen.luckyBoxes.length -
                         1].uId,
                   ),
                 );
           ...
           showDialog(
               ...
               builder: (BuildContext context) {
                 return AlertDialog(
                   ...
                   contentPadding: EdgeInsets.zero,
                   content: DialogLuckyBox(
                     coins: updatedListBoxes[updatedListBoxes.length - 1]
                         .coinns,
                     luckyBoxId: updatedListBoxes[updatedListBoxes.length - 1]
                         .boxId,
                     ownerBoxId: updatedListBoxes[updatedListBoxes.length - 1]
                         .ownerBoxId,
                     ownerBoxName: updatedListBoxes[updatedListBoxes.length - 1]
                         .ownerName,
                     removeController: luckyBoxRemovecontroller,
     ...
     luckyBoxRemovecontroller.stream.listen((
         List<LuckyBoxData> updatedListBoxes) {
       RoomScreen.updateLuckyBox.value = RoomScreen.updateLuckyBox.value + 1 ;
     });
     //topUserCached
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      subscriptions..add(ZegoUIKit()
        ...
        Future.delayed(const Duration(seconds: 3),()async{
           ZegoUIKit().sendInRoomMessage("انضم للغرفة");
        });
 ...
 //todo you shoulde remove setState here
  ...
  Future<void> loadAnimationGift(GiftData giftData) async {
    ...
    try {
      final videoItem = await Methods().getCachedSvgaImage(
          giftData.giftId, giftData.img);
  ...
  void loadMoreAnimationGifts() async {
    if (RoomScreen.listOfAnimatingGifts.isNotEmpty) {
      ...
      await loadAnimationEntro(RoomScreen.listOfAnimatingEntros[0].imgId,RoomScreen.listOfAnimatingEntros[0].imgUrl);
...
//massages
   ...
   if(result[messageContent]!= null){
     if (result[messageContent][message] == changeBackground) {
       ...
       if(result[messageContent][ownerId].toString() !=widget.room.ownerId.toString()){
         ...
         sendData =UserDataModel(profile:ProfileRoomModel(image:result[messageContent]['si'] ) ,
             name: result[messageContent]['sn'] ,
             level: LevelDataModel(senderImage:result[messageContent]['ssl'] ,
                 receiverImage:result[messageContent]['srl'] ),
             vip1:VipCenterModel(level:result[messageContent]['sv'] ) ) ;


         UserDataModel receiverData ;
         receiverData = UserDataModel(profile:ProfileRoomModel(image:result[messageContent]['ri'] ) ,
             name: result[messageContent]['rn'] ,
             level: LevelDataModel(senderImage:result[messageContent]['rsl'] ,
      ...
      showingEmojie(
       ...
       if(result[messageContent][isExpensive]) {
         if (RoomScreen.usersInRoom[sendId] == null) {
           ...
           RoomScreen.usersInRoom.putIfAbsent(sendId,
                   () => sendData);
         } else {
         ...
         if (RoomScreen.usersInRoom[receiverId] == null) {
           receiverData =
           await RemotlyDataSourceProfile().getUserData(userId: receiverId);
           RoomScreen.usersInRoom.putIfAbsent(receiverId,
                   () => receiverData);
         } else {
      ...
      log(result[messageContent]['userId'].toString());
       ...
       if(widget.myDataModel.id.toString() == result[messageContent]['userId']){
         ...
         RoomScreen.usersInRoom.forEach((key, value) {
           ...
           if (widget.myDataModel.id.toString() != widget.room.ownerId.toString()){
             ...
             RoomScreen.banedUsers.putIfAbsent(key,
                     () => key);
             RoomScreen.showMessageButton.value =false ;
           }
       ...
       ZegoUIKit().sendInRoomMessage(result[messageContent]['res']) ;
         ...
         showDialog(
             ...
             builder: (BuildContext context) {
               return  AlertDialog (
                 backgroundColor:Colors.transparent ,
                 contentPadding: EdgeInsets.zero,
                 content:  SucessLuckWidget(coins:result[messageContent]['co'].toString() ,),
         ...
         showDialog(
             ...
             builder: (BuildContext context) {
               return  const  AlertDialog(
                   backgroundColor:Colors.transparent ,
                   contentPadding: EdgeInsets.zero,
                   content:  ErrorLuckWidget(isNotLucky: false,)
                    ...
                    config: (widget.isHost
                      ...
                      return  ValueListenableBuilder<bool>(
                        ...
                        builder: (context, isMicrophoneEnabled, _) {
                          return  userAvatar(image: user.inRoomAttributes.value['img'].toString(),
                      ...
                      ..inRoomMessageViewConfig.itemBuilder =
                        ...
                        return ValueListenableBuilder(
                            ...
                            builder: (BuildContext context, dynamic value,
                              ...
                              if (RoomScreen.usersMessagesInRoom[message.user.id] == null) {
        ...
        return  Stack(
            children: [
              ...
              if (layoutMode == LayoutMode.hostTopCenter)
                ValueListenableBuilder<UserDataModel>(valueListenable: RoomScreen.topUserInRoom,
                    builder: (context,topUser,_){
                      return    hostTopCenterWidget(context ,
                          layoutMode ,topUser ,
                          widget.myDataModel ,
                          widget.room ) ;
                    }),
              ...
              ValueListenableBuilder<bool>(valueListenable: RoomScreen.showPK,
                  builder: (context,isShowPK,_){
                    if(isShowPK && layoutMode == LayoutMode.hostTopCenter){
                      return  Padding(padding: EdgeInsets.only(top: AppPadding.p45),
                        child:Align(
          ...
          return  Stack(
            ...
            return HeaderRoom(
              ...
              refreshRoom: refrashRoom, roomType: roomType, layoutMode: layoutMode,
                  ...
                  ValueListenableBuilder(
                      ...
                      builder: (context,edit,_){
                        if(RoomScreen.luckyBoxes.isNotEmpty){
                          return  luckyBox(luckyBoxRemovecontroller:luckyBoxRemovecontroller) ;
                        }else{
                          return const SizedBox() ;
                        }
                  ...
                  musicWidget(context , refrashRoom ,widget.room , controllerMusice , destroyMusic ),
              ...
              return Positioned(
                  ...
                  left: AppPadding.p36,
                  child:
                  showGiftBannerWidget(
                      isPlural: isPlural ,
                      sendDataUser: sendDataUser??widget.myDataModel.convertToUserObject(),
                      receiverDataUser: receiverDataUser ??widget.myDataModel.convertToUserObject(),
                      giftImage: giftBanner,
                      ownerId: ownerIdRoomBanner ??widget.room.ownerId.toString(),
 ...
 Widget showGiftBannerWidget(
  ...
  Widget showLuckyBannerWidget(
      {required UserDataModel sendDataUser,
        required String ownerId,required String coins,required bool isPassword  })  {
    Future.delayed(const Duration(seconds: 8)).then((value) {
    ...
    return AnimatedOpacity(
      ...
      child: InkWell(
        ...
        child: showLuckyBannerBodyWidget(sendDataUser: sendDataUser,coins:coins,ownerId: ownerId )
