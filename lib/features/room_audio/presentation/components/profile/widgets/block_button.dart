...
class BlockButton extends StatefulWidget {
  ...
  const BlockButton(
      {required this.userData, required this.roomData, super.key});
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton2(
          ...
          onChanged: (value) {
            if (value == StringManager.fiveMin.tr()) {
              BlocProvider.of<UsersInRoomBloc>(context).add(kickoutUser(
                  ownerId: widget.roomData.ownerId.toString(),
                  userId: widget.userData.id.toString(),
                  minutes: "5"));
            } else if (value == StringManager.fiftyMin.tr()) {
              BlocProvider.of<UsersInRoomBloc>(context).add(kickoutUser(
                  ownerId: widget.roomData.ownerId.toString(),
                  userId: widget.userData.id.toString(),
                  minutes: "15"));
            } else if (value == StringManager.thirtyMin.tr()) {
              BlocProvider.of<UsersInRoomBloc>(context).add(kickoutUser(
                  ownerId: widget.roomData.ownerId.toString(),
                  userId: widget.userData.id.toString(),
                  minutes: "30"));
            } else if (value == StringManager.sixtyMin.tr()) {
              BlocProvider.of<UsersInRoomBloc>(context).add(kickoutUser(
                  ownerId: widget.roomData.ownerId.toString(),
                  userId: widget.userData.id.toString(),
                  minutes: "60"));
            } else if (value == StringManager.twenyFourMin.tr()) {
              BlocProvider.of<UsersInRoomBloc>(context).add(kickoutUser(
                  ownerId: widget.roomData.ownerId.toString(),
                  userId: widget.userData.id.toString(),
                  minutes: "1440"));
